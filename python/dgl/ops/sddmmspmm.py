"""dgl sddmmspmm operator module."""
import sys

from ..backend import gsddmmspmm as gsddmmspmm_internal
from .. import backend as F

__all__ = ['gsddmmspmm']


def gsddmmspmm(g, op, reduce_op, lhs_data, rhs_data, imsg):
    ...
    ret = gsddmmspmm_internal(g._graph, op,
                         'sum' if reduce_op == 'mean' else reduce_op,
                         lhs_data, rhs_data, imsg)
...
def _gen_sddmmspmm_func(binary_op, reduce_op):
    name = "u_{}_e_{}".format(binary_op, reduce_op)
    docstring = """Generalized SDDMMSpMM function.
    It fuses two steps into one kernel.
    ...
    def func(g, x, y):
        return gsddmmspmm(g, binary_op, reduce_op, x, y, imsg)
    func.__name__ = name
...
def _register_sddmmspmm_func():
    """Register sddmmspmm functions
    ...
    for binary_op in ["add", "sub", "mul", "div", "copy_u", "copy_e"]:
        for reduce_op in ["sum", "max", "min", "mean"]:
            if binary_op.startswith("copy"):
                ...
                func = _gen_sddmmspmm_func(binary_op, reduce_op)
            setattr(sys.modules[__name__], func.__name__, func)
            __all__.append(func.__name__)


_register_sddmmspmm_func()
